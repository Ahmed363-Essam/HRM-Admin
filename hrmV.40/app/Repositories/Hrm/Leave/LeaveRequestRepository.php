...
class LeaveRequestRepository
{
                    ...
                    if ($user_check || $department_check) {
                        ...
                        if ($user) {
                            ...
                            if ($request->hasFile('attachment_file')) {
                                ...
                                $leaveRequest->attachment_file_id = $request->attachment_file;
                            }
                            $leaveRequest->save();
                            ...
                            if ($leaveRequest->user->manager_id != null) {
                                $this->sendChannelFirebaseNotification('user' . $leaveRequest->user->manager_id, 'leave_requested', null, route('leaveRequest.index'), $details['title'], $details['body'], null);

                                sendDatabaseNotification($leaveRequest->user->manager, $details);
                                try {
                                    Mail::to($leaveRequest->user->manager->email)->send(new LeaveRequestMail($leaveRequest->user, $details));
                                } catch (\Throwable $th) {
                                    Log::error($th);
                                }

                            } elseif ($leaveRequest->user->myHr() != null) {
                                $this->sendChannelFirebaseNotification('user' . $leaveRequest->user->myHr()->id, 'leave_requested', null, route('leaveRequest.index'), $details['title'], $details['body'], null);
                                sendDatabaseNotification($leaveRequest->user->myHr(), $details);
                                try {
                                    Mail::to($leaveRequest->user->myHr()->email)->send(new LeaveRequestMail($leaveRequest->user, $details));
                                } catch (\Throwable $th) {
                                    Log::error($th);
                                }
    ...
    public function approveOrRejectOrCancel($id, $status): bool
    {
        ...
        if ($leaveRequest) {
            ...
            $leaveRequest->save();
            $msg_body = 'Your ' . $leaveRequest->leave_from . ' To ' . $leaveRequest->leave_to . ' leave request has been reviewed';

            if ($status == 1) {
                //send notification to staff
                $this->sendChannelFirebaseNotification('user' . $leaveRequest->user->id, 'leave_approved', '', 'leave_approved', _trans('response.Leave Request Approved'), $msg_body, null);
                $this->approvedBy($leaveRequest);
            } elseif ($status == 6) {
                //send notification to staff
                $this->sendChannelFirebaseNotification('user' . $leaveRequest->user->id, 'leave_rejected', '', 'leave_rejected', _trans('response.Leave Request Rejected'), $msg_body, null);
                $this->rejectedBy($leaveRequest);
            } elseif ($status == 7) {
                //send notification to staff
                $this->sendChannelFirebaseNotification('user' . $leaveRequest->user->id, 'leave_cancelled', '', 'leave_cancelled', _trans('response.Leave Request Cancelled'), $msg_body, null);
                $this->cancelledBy($leaveRequest);
            } elseif ($status == 17) {
                //send notification to HR
                $this->sendChannelFirebaseNotification('user' . $leaveRequest->user->myHr->id, 'leave_referred', '', 'leave_referred', _trans('response.Leave Request Referred'), 'Leave Request Referred', null);
                $this->referredBy($leaveRequest);
            } else {
                //send notification to staff
                $this->sendChannelFirebaseNotification('user' . $leaveRequest->user->id, 'leave_rejected', '', 'leave_rejected', _trans('response.Leave Request Rejected'), 'Leave Request Rejected', null);
                $this->rejectedBy($leaveRequest);
            }
            return true;
        } else {
    ...
    public function availableLeave($request)
    {
        ...
        if ($user) {
            if (settings('leave_assign') == 1) {
                $assignedLeaveList = AssignLeave::where(['company_id' => $this->companyInformation()->id, 'user_id' => $user->id])->get();
            } else {
    ...
    public function storeDailyLeave($request)
    {
        ...
        if ($daily_leave->save()) {
            ...
            if ($daily_leave->user->manager_id != null) {
                $this->sendCustomFirebaseNotification($daily_leave->user->manager, 'notice', '', 'daily_leave', 'New ' . $formated_leave_type . ' Request', $notify_body);
                sendDatabaseNotification($daily_leave->user->manager, $details);
            }
            if ($daily_leave->user->myHr() != null) {
                $this->sendCustomFirebaseNotification($daily_leave->user->myHr(), 'notice', '', 'daily_leave', 'New ' . $formated_leave_type . ' Request', $notify_body);
    ...
    public function dailyLeaveNotification($daily_leave, $status, $route = null)
    {
        ...
        $this->sendCustomFirebaseNotification($daily_leave->user, 'notice', '', 'daily_leave', $formated_leave_type . ' Request ' . $status, $notify_body);
    ...
    function dailyLeaveTable($request)
    {
        try {
            // Log::info($request);
            ...
            if ($request->from && $request->to) {
                ...
                $today = date('Y-m-d');
                $leaveRequest = $leaveRequest->where('leave_from', '<=', $today)->where('leave_to', '>=', $today);
            }
            ...
            return [
                'data' => $data->map(function ($data) {
                    $leaveDates = '<b>From: </b>' . $data->leave_from . '<br><b>To: </b>' . $data->leave_to;

                    $available_days = @resolve(LeaveService::class)->leaveSummary($data)->original['data'];
                    $leaveName = '<b class="highlight">' . @$data->assignLeave->type->name . '</b>';
                    $type = $leaveName . '<br><b>Days: </b>' . $data->days . '<br>' . $leaveDates;
                    ...
                    foreach ($available_days['available_leave'] as $availables) {
                        ...
                        <span><b>Total:</b> ' . $availables['total_leave'] . ' | <b>Left:</b> ' . $availables['left_days'] . '</span>
                    </div>';
    ...
    function LeaveReportTable($request)
    {
        try {
            ...
            $leaveRequest->where('status_id', 1);
            ...
            return [
                'data' => $data->map(function ($data) {
                    $leaveName = '<b class="highlight">' . @$data->assignLeave->type->name . '</b>';
                    $approvals = '';
                    $approvals .= @AuthorInfo::where(['authorable_type' => get_class($this->leaveRequest), 'authorable_id' => $data->id])->first()->referredUser->name ? _trans('common.Manager Approved') . '<br>' : '';
                    $approvals .= @AuthorInfo::where(['authorable_type' => get_class($this->leaveRequest), 'authorable_id' => $data->id])->first()->approveUser->name ? _trans('common.HR Approved') : '';
        ...
        try {
            ...
            return [
                'data' => $data->map(function ($data) {
                    ...
                    $available_leave = '';
                    foreach ($available_days['available_leave'] as $availables) {
                        $htm = '<div class="d-flex justify-content-between text-center gap-4">
                                <span><b>' . $availables['type'] . ':</b></span>
                                <span><b>Total:</b> ' . $availables['total_leave'] . ' | <b>Left:</b> ' . $availables['left_days'] . '</span>
                        </div>';
                        $available_leave .= $htm;
