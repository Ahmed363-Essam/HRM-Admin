        ...
        if ($user) {
            ...
            if ($isIpRestricted) {
                ...
                if (count($attendance_status) > 0) {
                    ...
                    $check_in->check_in_location = getGeocodeData($request->check_in_latitude,$request->check_in_longitude);
    ...
    public function liveLocationStore($request)
    {
        try {
            ...
            foreach ($request->locations as $key => $location) {
                ...
                $locationLog->date = date('Y-m-d', strtotime($datetime));
                $locationLog->time = date('H:i:s', strtotime($datetime));
                $locationLog->created_at = Carbon::createFromFormat('Y-m-d H:i:s.u', $datetime)->toDateTimeString();
