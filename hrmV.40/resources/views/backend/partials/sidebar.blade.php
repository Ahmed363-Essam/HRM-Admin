    ...
    <div class="sidebar-menu">
        <div class="sidebar-menu-section">
            ...
            <ul class="sidebar-dropdown-menu parent-menu-list">
                ...
                {{-- Modules/Saas/Resources/views/backend/saas_sidebar.blade.php --}}
                @if (isModuleActive('Saas'))
                    @include('saas::backend.saas_sidebar')
                @endif
                <li class="sidebar-menu-item">
                    <a href="javascript:void(0)"
                        class="parent-item-content has-arrow  {{ menu_active_by_route(
                            [
                                'models.index', 'brands.index', 'machines.index', 'packages.index', 'institutions.index', 'services.index',
                                'saas.company.list','branches.index','documents.index','documents.types.index'
                                ]) }}">
                    ...
                    <ul class="child-menu-list  {{ set_active(['documents/*','documents/types', 'documents/list','admin/saas/companies','branches']) }}">
                        ...
                        @if (config('app.mood') === 'Saas' && isModuleActive('Saas'))
                            @if (in_array(url('/'), config('tenancy.central_domains')))
                                <li class="sidebar-menu-item {{ menu_active_by_route(['saas.company.list']) }}">
                                    <a href="{{ route('saas.company.list') }}"
                                        class="{{ set_active('admin/saas/companies') }}">
                                        <span>
                ...
                @if (hasPermission('leave_menu'))
                    <li class="sidebar-menu-item {{ set_menu([route('leave.index'), route('assignLeave.index')]) }}">
                        ...
                        <ul
                            class="child-menu-list {{ set_active(['hrm/leave*', 'hrm/leave/assign*', 'hrm/leave/request*', 'dashboard/user/leave-balance*']) }}">
                            ...
                            @if (hasPermission('daily_leave_read'))
                                <li class="nav-item {{ menu_active_by_route(['dailyLeave.index']) }}">
                ...
                @endif
                @if (hasPermission('conference_read') && isModuleActive('VideoConference'))
                    <li class="sidebar-menu-item ">
                ...
                @if (hasPermission('general_settings_read') ||  2==2)
                    <li class="sidebar-menu-item">
                        <a href="javascript:void(0)"
                        ...
                        <ul
                            class="child-menu-list  {{ set_active(['admin/settings*', 'admin/settings/list', 'admin/settings/leave*', 'admin/settings/ip-configuration*', 'company/settings', 'admin/settings/app-setting/dashboard', 'admin/settings/language-setup', 'admin/content/list*']) }}">
                            ...
                            @if (hasPermission('content_menu'))
                                <li class="nav-item {{ menu_active_by_route('content.list') }}">
                                    <a href="{{ route('content.list') }}"
                                        class=" {{ set_active('admin/content/*') }}">
                                        <span>{{ _trans('common.Content') }}</span>
                                    </a>
                                </li>
                            @endif
